...
class DVRK_Reacher(Env, utils.EzPickle):
    ...
    def step(self, a):
        ...
        if GT_RECOVERY and self.caution_zone(self._low_dim_state):
            ...
            a = process_action(self._low_dim_state,
                               self.safe_action(self._low_dim_state))
        ...
        cur_cost = self.step_cost(
            self._low_dim_state, a
        )  # QUESTION: is _low_dim_state the next state? Because thats not ideal right then this is C(s', a) instead of C(s, a)
    ...
    def reset(self):
        if self.pointbot_dynamics:
            ...
            if self.ring:
                ...
                self.state = (np.array(GOAL_STATE) + 0.03 * np.array(
                    [np.cos(theta[0]), np.sin(theta[0]), 0])) * 100
        else:
            pos = START_STATE  # position in (m)
            if self.ring:
                theta = np.random.uniform(0, 2 * np.pi, 1)
                pos = GOAL_STATE + 0.03 * np.array(
                    [np.cos(theta[0]), np.sin(theta[0]), 0])  # position in (m)
            rot = [0.0, 0.0, 0.0]  # Euler angles
            jaw = [0 * np.pi / 180.]  # jaw angle in (rad)
            quat = U.euler_to_quaternion(
                rot, unit='deg')  # convert Euler angles to quaternion
    ...
    def _execute_action(self, a):
        ...
        quat = U.euler_to_quaternion(
            [0, 0, 0], unit='deg')  # convert Euler angles to quaternion
        jaw = [0 * np.pi / 180.]  # jaw angle in (rad)
    ...
    def _state(self):
        ...
        if self.image_obs:
            img_color, img_depth, img_point = self.zivid.capture_3Dimage(
                color='RGB')  # 7~10 fps
            img_color = resize(
                img_color,
                (img_color.shape[0] // 30, img_color.shape[1] // 30),
                anti_aliasing=True)
            return img_color
...
if __name__ == "__main__":
    ...
    for ep in range(num_demos):
        ...
        episode_steps = 0
        SUBGOAL_STATE = SUBGOAL_STATE_1 if np.random.random(
        ) < 0.5 else SUBGOAL_STATE_2
        # Randomize z:
        SUBGOAL_STATE[2] = np.random.uniform(100 * BOUNDSZ[0],
                                             100 * BOUNDSZ[1])
        while not done:
            # Sample a point inside the obstacle and try to hit it
            obs_point = np.array([
                np.random.uniform(*OBS_X),
                np.random.uniform(*OBS_Y),
                np.random.uniform(*OBS_Z)
            ])
            ...
            if episode_steps < 4:
                a = MAX_FORCE * (
                    np.array(SUBGOAL_STATE) * 100 -
                    env._low_dim_state) / (np.linalg.norm(
                        np.array(SUBGOAL_STATE) * 100 - env._low_dim_state))
            else:
                a = MAX_FORCE * (
                    np.array(GOAL_STATE) * 100 - env._low_dim_state) / (
                        np.linalg.norm(
                            np.array(GOAL_STATE) * 100 - env._low_dim_state))
            ...
            mask = float(not done)
            demo_transitions.append((process_obs(state), action, constraint,
                                     process_obs(next_state), mask))
            demo_transitions_lowdim.append((lowdim_state, action, constraint,
                                            next_lowdim_state, mask))
            state = next_state
    ...
    pickle.dump({
        "images": demo_transitions,
        "lowdim": demo_transitions_lowdim
