...
def create_object_xml(filename,
    ...
    if finger_sensors:
        ...
        ET.SubElement(
            sensor_frame, "touch", name="finger1_sensor", site="finger1_surf")
        ET.SubElement(
            sensor_frame, "touch", name="finger2_sensor", site="finger2_surf")
    else:
        sensor_frame = None
    ...
    for i in range(num_objects):
        if load_dict_list == None:
            dict = {}
            if i == 1:
                color1 = dict['color1'] = np.array([1, 0, 0])
                color2 = dict['color2'] = np.array([1, 0, 0])
            else:
                color1 = dict['color1'] = np.array(
                    [1, 1, np.random.uniform(0, 0.1)])
                color2 = color1.copy()
                # color1 = dict['color1'] = np.random.uniform(0.45, 1., 3)
                # color2 = dict['color2'] = np.random.uniform(0.45, 1., 3)
            ...
            if object_meshes is not None:
                dict['chosen_mesh'] = chosen_mesh = random.choice(
                    object_meshes)
        ...
        print('using friction=({}, {}, {}), object mass{}'.format(
            f_sliding, f_torsion, f_rolling, object_mass))
        if object_meshes is not None:
            ...
            if chosen_mesh not in loaded_meshes:
                ...
                convex_hull_files = [
                    x for x in stl_files if 'Shape_IndexedFaceSet' in x
                ]
                object_file = [
                    x for x in stl_files if x not in convex_hull_files
                    and 'Lamp' not in x and 'Camera' not in x and 'GM' not in x
                ][0]
                ...
                if chosen_mesh in [
                        'Knife', 'Fork', 'Spoon'
                ]:  #should add a more extensible way to handle different rescale rules
                    ...
                    if chosen_mesh == 'Knife':
                        ...
                        'Bowl', 'ElephantBowl', 'GlassBowl', 'LotusBowl01',
                        'RuggedBowl'
                ]:
                    ...
                    scale = [maxlen / max_length for _ in range(2)
                             ] + [maxlen / (maxz - minz) * 0.75]
                ...
                mass_per_elem, n_cvx_files = object_mass / (
                    1 + len(convex_hull_files)), len(convex_hull_files)
                loaded_meshes[chosen_mesh] = (object_pos, mass_per_elem,
                                              n_cvx_files)

                ET.SubElement(
                    assets,
                    "mesh",
                    name=chosen_mesh + "_mesh",
                    file=object_file,
                ...
                for n, c_file in enumerate(convex_hull_files):
                    ET.SubElement(
                        assets,
                        "mesh",
                        name=chosen_mesh + "_convex_mesh{}".format(n),
                        file=c_file,
                        scale="{} {} {}".format(scale[0], scale[1], scale[2]))

            else:
                object_pos, mass_per_elem, n_cvx_files = loaded_meshes[
                    chosen_mesh]

            pos_str = "{} {} {}".format(object_pos[0], object_pos[1],
                                        object_pos[2])
            ...
            if obj_classname is not None:
                ...
                obj = ET.SubElement(
                    world_body, "body", name=obj_string, pos=pos_str)
            ...
            ET.SubElement(
                ...
                "geom",
                type="mesh",
                mesh=chosen_mesh + "_mesh",
                rgba="{} {} {} 1".format(color1[0], color1[1], color1[2]),
                mass="{}".format(mass_per_elem),
                contype="0",
                conaffinity="0")
            #contact meshes
            for n in range(n_cvx_files):
                ET.SubElement(
                    ...
                    "geom",
                    type="mesh",
                    mesh=chosen_mesh + "_convex_mesh{}".format(n),
                    rgba="0 1 0 0",
                    mass="{}".format(mass_per_elem),
                    margin="0.00005",
                    contype="7",
                    conaffinity="7",
                    friction="{} {} {}".format(f_sliding, f_torsion,
                                               f_rolling))
            ...
            if obj_classname is not None:
                ...
                obj = ET.SubElement(
                    world_body, "body", name=obj_string, pos="0 0 0")
            ...
            if tall_objs:
                ET.SubElement(
                    ...
                    "geom",
                    type="box",
                    size="{} {} {}".format(0.03, 0.03, 0.12),
                    rgba="{} {} {} 1".format(color1[0], color1[1], color1[2]),
                    mass="{}".format(object_mass),
                    contype="7",
                    conaffinity="7",
                    friction="{} {} {}".format(f_sliding, f_torsion,
                                               f_rolling))
            ...
            ET.SubElement(
                ...
                "geom",
                type="box",
                size="{} {} {}".format(block_width, l1, block_height),
                rgba="{} {} {} 1".format(color1[0], color1[1], color1[2]),
                mass="{}".format(object_mass),
                contype="7",
                conaffinity="7",
                friction="{} {} {}".format(f_sliding, f_torsion, f_rolling))
            ET.SubElement(
                obj,
                "geom",
                pos="{} {} 0.0".format(l2, pos2),
                type="box",
                size="{} {} {}".format(l2, block_width, block_height),
    ...
    xml_str = minidom.parseString(ET.tostring(
        tree.getroot(), 'utf-8')).toprettyxml(indent="    ")

    xml_str = xml_str.splitlines()[1:]
    xml_str = "\n".join(xml_str)

    with open(xmldir + "/auto_gen_objects{}.xml".format(os.getpid()),
              "w") as f:
        f.write(xml_str)
...
def clean_xml(filename):
    ...
    obj_file = '/'.join(filename.split('/')
                        [:-1]) + '/auto_gen_objects{}.xml'.format(xml_number)
...
def create_root_xml(filename):
    ...
    with open(filename) as f:
        for i, l in enumerate(f):
            if 'begin_auto_replace' in l:
                ...
                if not replace_done:
                    newlines.append(
                        '        <!--begin auto replaced section -->\n')
                    newlines.append(
                        '        <include file="auto_gen_objects{}.xml"/>\n'.
                        format(os.getpid()))
                    newlines.append(
                        '        <!--end auto replaced section -->\n')
                    replace_done = True
    ...
    outfile = '/'.join(str.split(
        filename, '/')[:-1]) + '/auto_gen{}.xml'.format(os.getpid())
...
if __name__ == '__main__':
    params = imp.load_source(
        'hyper',
        "/home/frederik/Documents/catkin_ws/src/visual_mpc/pushing_data/cartgripper_genobj/hyperparams.py"
    )
