...
ASSET_BASE_DIR = osp.join('/'.join(os.path.abspath(os.path.dirname(__file__)).split('/')[:-1]), 'mjc_models')
    ...
    for i in range(num_objects):
        if load_dict_list == None:
            ...
            if i == 1:
                ...
                color1 = dict['color1'] = np.array([1, 1, np.random.uniform(0, 0.1)])
                color2 = color1.copy()
        ...
        if object_meshes is not None:
            ...
            if tall_objs:
                ET.SubElement(obj, "geom", type="box", size="{} {} {}".format(0.03, 0.03, 0.12),
