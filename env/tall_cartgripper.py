...
class TallCartgripperEnv(CartgripperRotGraspEnv):
    ...
    def _next_qpos(self, action):
        ...
        target, self._gripper_closed = no_rot_dynamics(self._previous_target_qpos, action,
                                                          self._gripper_closed, gripper_z, z_thresh, self._hp.reopen,
                                                          delta_z_cond)
        target = clip_target_qpos(target, self._hp.pos_lower_bound, self._hp.pos_upper_bound)
        return target
    ...
    def get_grasp_action(self, idx=1, noise_std=0.00, drop=False):
        ...
        gain = 1
        if np.abs(position[1] - 0.15) > 0.02 and np.abs(position[0] - obj_position[0]) > 0.04:
            print(0)
    ...
    def generate_goal_image(self):
        ...
        actions = np.vstack([np.tile(np.array([0.02, 0, 0, 0]), (5, 1)), actions])
        for ac in actions:
           obs = self.step(ac)
        # im = obs[0]['images'][0]
        im = self.render()[0]
