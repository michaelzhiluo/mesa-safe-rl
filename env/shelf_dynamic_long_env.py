...
class ShelfDynamicLongEnv(BaseMujocoEnv):
    ...
    def expert_action(self, t, noise_std=0.0, demo_quality='high'):
        ...
        if t < 2:
            return np.clip([0, 0, 0.03, 0] + np.random.randn(self._adim) * noise_std, self.ac_low, self.ac_high) 
...
if __name__ == '__main__':
    ...
    for t in range(25):
        # if t < 7:
        #     ac = env.expert_action(2, noise_std=0.01, demo_quality='low')
        # else:
        #     ac = env.expert_action(t, noise_std=0.2, demo_quality='low')

        ac = env.expert_action(t, noise_std=0.005)
        # ac = [0, 0, 0, 0.6]
