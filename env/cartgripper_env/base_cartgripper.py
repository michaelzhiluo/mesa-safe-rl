...
class BaseCartgripperEnv(BaseMujocoEnv):
    ...
    def __init__(self, env_params_dict, reset_state = None):
        ...
        self._reset_xml = create_object_xml(self.base_filename, _hp.num_objects, _hp.object_mass,
                                            self.friction_params, self.object_meshes, _hp.finger_sensors,
                                            _hp.maxlen, _hp.minlen, self.reset_xml,
    ...
    def reset(self, reset_state=None, randomize_objects=True, gen_goal=False):
        if randomize_objects:
            self._reset_xml = create_object_xml(self.base_filename, self._hp.num_objects, self._hp.object_mass,
                                            self.friction_params, self.object_meshes, self._hp.finger_sensors,
                                            self._hp.maxlen, self._hp.minlen, self.reset_xml,
                                            self._hp.obj_classname, cube_objs=self._hp.cube_objects,
                                            block_height=self._hp.block_height, tall_objs=self._hp.tall_objects)
        if gen_goal:
    ...
    def _get_obs(self, finger_sensors):
        ...
        obs['constraint_violated'] = np.linalg.norm(obs['object_poses'][0] 
                - self._hp.object_poses[0]) > 1e-2 # check if first object has moved greater than 1e-2 (1 cm I think)
