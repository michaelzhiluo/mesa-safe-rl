...
while i_demos < args.num_demos:
    ...
    while not done: 
        if args.constraint_demos:
            if time_seed < 0.5:
                if t < 7:
                    action = env.expert_action(2, noise_std=0.01, demo_quality='low')
                else:
                    action = env.expert_action(t, noise_std=0.2, demo_quality='low')
            else:
                if t < 16:
...
if args.constraint_demos:
    ...
    if not args.save_rollouts:
        pickle.dump(demo_transitions, open(os.path.join("demos/shelf_dynamic_long", f_name), "wb") )
    else:
        pickle.dump(demo_rollouts, open(os.path.join("demos/shelf_dynamic_long", f_name), "wb") )
else:
    f_name = "task_demos"
    if args.save_rollouts:
        f_name += "_rollouts"
