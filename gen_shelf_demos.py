        ...
        if args.constraint_demos:
            if args.vismpc_train_data:
                action = env.expert_action(
                    noise_std=0.05, demo_quality=args.demo_quality)
            else:
                if episode_steps > 3:
                    action = env.expert_action(
                        noise_std=0.25, demo_quality=args.demo_quality)
                else:
                    action = env.expert_action(
                        noise_std=0.1, demo_quality=args.demo_quality)
        else:
            action = env.expert_action(
                noise_std=0.01, demo_quality=args.demo_quality)
