...
from env.shelf_long_env import ShelfLongEnv
import pickle
import time
...
def process_obs(obs):
    ...
    im = obs
    im = cv2.resize(im, (agent_img_width, agent_img_height),
                                        interpolation=cv2.INTER_AREA)
    im = np.transpose(im, (2, 0, 1))
    return im

parser = argparse.ArgumentParser(description='PyTorch Soft Actor-Critic Args')
parser.add_argument('--env-name', default="ShelfEnv",
                    help='Mujoco Gym environment (default: ShelfEnv')
parser.add_argument('--start_steps', type=int, default=5000, metavar='N', # TODO: think about what this is approperiate to be...maybe even lower, or make it higher
                                                                          # so can explore sufficiently after the demos are over??
                    help='Steps sampling random actions (default: 10000)')
parser.add_argument('--num_demos', type=int, default=250, metavar='N',
...
parser.add_argument('--demo_filter_constraints', action="store_true", 
...
parser.add_argument('--use_constraint_penalty', action="store_true",
...
while i_demos < args.num_demos:
    ...
    while not done: 
        if args.constraint_demos:
            ...
            action = env.expert_action(noise_std=0.001, demo_quality=args.demo_quality)
...
if args.constraint_demos:
    ...
    if not args.save_rollouts:
        if not args.vismpc_train_data:
            pickle.dump(demo_transitions, open(os.path.join("demos/shelf_long", f_name), "wb") )
        else:
            pickle.dump({'obs': obs_seqs, 'ac': ac_seqs, 'constraint': constraint_seqs}, 
                        open(os.path.join("demos/shelf_long", f_name), "wb") )
    else:
        pickle.dump(demo_rollouts, open(os.path.join("demos/shelf_long", f_name), "wb") )
else:
    f_name = "task_demos"
    if args.save_rollouts:
        f_name += "_rollouts"
