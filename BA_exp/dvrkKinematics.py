...
class dvrkKinematics():
    ...
    def pose_to_joint(self, pos, rot, method='analytic'):
        if pos == [] or rot == []:
            ...
            T = np.matrix(self.pose_to_transform(
                pos, rot))  # current transformation
            joint = self.ik(T, method)
        return joint

    @classmethod
    def transform(cls, a, alpha, d, theta,
                  unit='rad'):  # modified DH convention
        if unit == 'deg':
            alpha = np.deg2rad(alpha)
            theta = np.deg2rad(theta)
        T = np.array([[np.cos(theta), -np.sin(theta), 0, a], [
            np.sin(theta) * np.cos(alpha),
            np.cos(theta) * np.cos(alpha), -np.sin(alpha), -np.sin(alpha) * d
        ], [
    ...
    def ik(self, T, method='analytic'):
        if method == 'analytic':
            ...
            joint = [
                ik_sol[0, 0], ik_sol[0, 1], ik_sol[0, 2], ik_sol[0, 3],
                ik_sol[0, 4], ik_sol[0, 5]
            ]
    ...
    def ik_orientation(cls, q1, q2, Rb):
        R03 = np.array(
            [[-np.sin(q1) * np.sin(q2), -np.cos(q1),
              np.cos(q2) * np.sin(q1)], [-np.cos(q2), 0, -np.sin(q2)],
