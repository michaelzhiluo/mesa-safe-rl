...
class MAMLRAWR(object):
    ...
    def select_action(self, state, eval=False, policy=None):
        ...
        if eval is False:
            ...
            action = mu + self.action_sigma * torch.empty_like(mu).normal_()
    ...
    def value_function_loss_on_batch(self, value_function, state_batch, mc_reward_batch, inner: bool = False):
        ...
        for i, (train_task_idx, inner_buffer, outer_buffer) in enumerate(zip(range(self.num_tasks), inner_buffers, outer_buffers)):
            ...
            with higher.innerloop_ctx(vf, opt, override={'lr': [F.softplus(l) for l in self._value_lrs]}, copy_initial_weights=False) as (f_value_function, diff_value_opt):
                for step in range(self._maml_steps):
                    loss, value_inner, mc_inner, mc_std_inner = self.value_function_loss_on_batch(f_value_function, state_batch, mc_reward_batch, inner=True)
                ...
                meta_value_function_loss, value, mc, mc_std = self.value_function_loss_on_batch(f_value_function, meta_state_batch, meta_mc_reward_batch)
    ...
    def eval_adaptation(self, policy, memory):
        ...
        for step in range(20):
            ...
            vf_loss, _, _, _ = self.value_function_loss_on_batch(vf, state_batch, mc_reward_batch, inner=True)
            vf_loss.backward()
            opt.step()
            opt.zero_grad()
            ...
            if step+1 in log_steps:
                self.plot(policy, self.updates, folder_prefix="/" + str(step+1) + "/", critic=vf)
    ...
    def update_parameters(self, ep=None, memory=None, policy=None, critic=None, lr=None, batch_size=None, training_iterations=None, plot=None):
        ...
        vf_loss, value_inner, mc_inner, mc_std_inner = self.value_function_loss_on_batch(vf, state_batch, mc_reward_batch, inner=True)
        vf_loss.backward()
        self.online_adapt_value_opt.step()
        self.online_adapt_value_opt.zero_grad()

        self._adaptation_policy.train() 
        actor_loss, adv, weights, adv_loss = self.adaptation_policy_loss_on_batch(self._adaptation_policy,
